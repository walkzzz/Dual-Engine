...
func init() {
	if endpoint := os.Getenv("LOCAL_ENDPOINT"); endpoint != "" {
		...
		// Check if this is a DashScope-compatible endpoint
		if strings.Contains(endpoint, "dashscope") {
			// Register models with local provider
			for _, model := range DashScopeModels {
				localModel := model
				localModel.Provider = ProviderLocal
				SupportedModels[model.ID] = localModel
			}
			ProviderPopularity[ProviderLocal] = 0
			viper.SetDefault("providers.local.apiKey", os.Getenv("DASHSCOPE_API_KEY"))
			logging.Info("Configured DashScope provider via LOCAL_ENDPOINT")
			return
		}
